...
import { aspectRatios, type AspectRatio } from '../assets/assets.js';
import { button } from 'motion/react-client';

const AppRatioSelector = ({value,onChange}:{value:AspectRatio; onChange:(ratio: AspectRatio)=>void }) => {

    const iconMap= {
        "16:9":<RectangleHorizontal className='size-6'/>,
        "1:1":<Square className='size-9'/>,
        "9:16":<RectangleVertical className='size-6'/>
    } as Record<AspectRatio, React.ReactNode>;

  return (
    <div className='space-y-3 dark'>
        <label htmlFor="" className='block text-sm font-medium text-zinc-200'>Aspect Ratio</label>
...
export default AppRatioSelector
